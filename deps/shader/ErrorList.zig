...
pub fn add(self: *ErrorList, loc: Token.Loc, comptime format: []const u8, args: anytype, note: ?ErrorMsg.Note) !void {
...
pub fn createNote(self: *ErrorList, loc: ?Token.Loc, comptime format: []const u8, args: anytype) !ErrorMsg.Note {
    return .{
        .loc = loc,
        .msg = try std.fmt.allocPrint(self.arena.allocator(), comptime format, args),
    };
