...
pub fn Quat(comptime Scalar: type) type {
    return extern struct {
        ...
        pub inline fn create(x: T, y: T, z: T, w: T) Quat(T) {
        ...
        pub inline fn inverse(q: *const Quat(T)) Quat(T) {
            const s = 1 / q.len2();
            return create(-q.v.x() * s, -q.v.y() * s, -q.v.z() * s, q.v.w() * s);
        }
        ...
        pub inline fn fromAxisAngle(axis: Axis, angle: T) Quat(T) {
            ...
            return create(s * axis.x(), s * axis.y(), s * axis.z(), math.cos(halfAngle));
        }
        ...
        pub inline fn add(a: *const Quat(T), b: *const Quat(T)) Quat(T) {
            return create(a.v.x() + b.v.x(), a.v.y() + b.v.y(), a.v.z() + b.v.z(), a.v.w() + b.v.w());
        }
        ...
        pub inline fn rotateX(q: *const Quat(T), angle: T) Quat(T) {
            ...
            return create(qx * bw + qw * bx, qy * bw + qz * bx, qz * bw - qy * bx, qw * bw - qx * bx);
        }
        ...
        pub inline fn rotateY(q: *const Quat(T), angle: T) Quat(T) {
            ...
            return create(qx * bw - qz * by, qy * bw + qw * by, qz * bw + qx * by, qw * bw - qy * by);
        }
        ...
        pub inline fn rotateZ(q: *const Quat(T), angle: T) Quat(T) {
            ...
            return create(qx * bw - qy * bz, qy * bw + qx * bz, qz * bw + qw * bz, qw * bw - qz * bz);
        }
        ...
        pub inline fn slerp(a: *const Quat(T), b: *const Quat(T), t: T) Quat(T) {
            ...
            return create(scale0 * ax + scale1 * bx, scale0 * ay + scale1 * by, scale0 * az + scale1 * bz, scale0 * aw + scale1 * bw);
        }

        /// Calculates the conjugate of the given quaternion.
        pub inline fn conjugate(q: *const Quat(T)) Quat(T) {
            return create(-q.v.x(), -q.v.y(), -q.v.z(), q.v.w());
        }
        ...
        pub inline fn fromEuler(x: T, y: T, z: T) Quat(T) {
            ...
            return create(xRet, yRet, zRet, wRet);
        }
        ...
        pub inline fn normalize(q: *const Quat(T)) Quat(T) {
            ...
            if (length > 0.00001) {
                return create(q0 / length, q1 / length, q2 / length, q3 / length);
            } else {
