...
pub fn Ray3(comptime Scalar: type) type {
    ...
    const PP: type = switch (P) {
        ...
        f32 => f64,
        f64 => f128,
        else => @compileError("Expected f16, f32, f64, found '" ++ @typeName(P) ++ "'"),
    };
    ...
    const Vec4P = switch (Vec3P) {
        ...
        else => @compileError("Expected Vec3, Vec3h, Vec3d, found '" ++ @typeName(Vec3P) ++ "'"),
    };
    ...
    return extern struct {
        ...
        pub inline fn triangleIntersect(ray: *const Ray3(P), va: *const Vec3P, vb: *const Vec3P, vc: *const Vec3P, backface_culling: bool) ?Hit {
            ...
            if (u == 0.0 or v == 0.0 or w == 0.0) {
                const cxby: PP = @as(PP, @floatCast(cx)) * @as(PP, @floatCast(by));
                const cybx: PP = @as(PP, @floatCast(cy)) * @as(PP, @floatCast(bx));
                u = @floatCast(cxby - cybx);

                const axcy: PP = @as(PP, @floatCast(ax)) * @as(PP, @floatCast(cy));
                const aycx: PP = @as(PP, @floatCast(ay)) * @as(PP, @floatCast(cx));
                v = @floatCast(axcy - aycx);

                const bxay: PP = @as(PP, @floatCast(bx)) * @as(PP, @floatCast(ay));
                const byax: PP = @as(PP, @floatCast(by)) * @as(PP, @floatCast(ax));
            ...
            var hit: Hit = Vec4P.create(undefined, undefined, undefined, math.inf(f32));
