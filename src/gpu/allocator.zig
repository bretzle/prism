...
pub const Allocator = union(enum) {
    ...
    pub fn initOffsetAllocator(allocator: std.mem.Allocator, size: u32, max_allocations: ?u32) !Allocator {
        return .{
            .offset_allocator = try OffsetAllocator.init(allocator, size, max_allocations),
        };
    }

    pub fn initDedicatedBlockAllocator(size: u64) !Allocator {
        return .{
            .dedicated_block_allocator = try DedicatedBlockAllocator.init(size),
        };
    }
    ...
    pub fn allocate(self: *Allocator, size: u32) Error!Allocation {
        return switch (self.*) {
...
pub const DedicatedBlockAllocator = struct {
    ...
    pub fn init(size: u64) !DedicatedBlockAllocator {
        return .{
            .size = size,
            .allocated = 0,
        };
    ...
    pub fn allocate(self: *DedicatedBlockAllocator, size: u32) Error!Allocation {
        if (self.allocated != 0) {
...
pub const OffsetAllocator = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, size: u32, max_allocations: ?u32) !OffsetAllocator {
    ...
    pub fn allocate(self: *OffsetAllocator, size: u32) Error!Allocation {
